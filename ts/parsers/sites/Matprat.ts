...
export class Matprat extends ReplacementParser {
    ...
    total_time() {
        const ele = this.querySelector(
            `span[data-epi-property-name="RecipeTime"]`
        );
        return normalizeElement(ele);
    }
    ...
    yields() {
        ...
        const ele2 = this.querySelector(
            "div.recipe-adjust-servings__original-serving"
        );
    ...
    ingredients() {
        const eles = this.querySelectorAll(
            `.ingredients-list li[itemprop="ingredient"]`
        );
