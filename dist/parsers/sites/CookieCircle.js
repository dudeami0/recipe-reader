...
export class CookieCircle extends ReplacementParser {
    ...
    total_time() {
        const eles = this.querySelectorAll("ul.single-method-overview__times li");
        const ele = eles.filter((ele) => {
            const text = ele.querySelector("span:nth-child(1)");
            return (text?.textContent?.toLowerCase().indexOf("total time:") !== -1);
        })[0];
        return normalizeElement(ele);
    }
    ingredients() {
        const eles = this.querySelectorAll(`div.single-ingredients__group[data-unit="metric"] ` +
...
//# sourceMappingURL=CookieCircle.js.map